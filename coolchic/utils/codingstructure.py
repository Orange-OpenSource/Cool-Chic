...
class CodingStructure:
    ...
    def __post_init__(self):
        ...
        self.frames = self.compute_coding_struct(self.n_frames, self.intra_pos, self.p_pos)
    ...
    def pretty_structure_diagram(self) -> str:
        ...
        all_x_pos = [round(x / (self.n_frames - 1) * _LENGTH_PRINT) for x in range(self.n_frames)]
        # print(all_x_pos)
        ...
        for depth in range(self.get_max_depth() + 1):
            ...
            for frame in self.get_all_frames_of_depth(depth):
                ...
                # Only past ref, \----> on the left, only whitespace on the right
                    ...
                    spacing = (
                        all_x_pos[frame.index_references[0]] + len_left_ref_str - current_x_pos
                    )
                    left_arrow_len = (
                ...
                # Past and future \---> on the left, <---/ on the right
                    ...
                    current_x_pos += spacing + left_arrow_len + right_arrow_len + len(frame_str)
