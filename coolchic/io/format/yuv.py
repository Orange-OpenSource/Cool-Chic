...
def write_yuv(
    ...
    with open(file_path, "ab" if append else "wb") as f_out:
        raw_data.tofile(f_out)
...
def yuv_dict_to_device(yuv: DictTensorYUV, device: torch.device) -> DictTensorYUV:
...
def convert_444_to_420(yuv444: Tensor) -> DictTensorYUV:
    ...
    # uv = F.interpolate(yuv444[:, 1:3, :, :], scale_factor=(0.5, 0.5), mode="bicubic")
    uv = F.avg_pool2d(yuv444[:, 1:3, :, :], kernel_size=2, stride=2)
