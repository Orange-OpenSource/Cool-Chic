...
from coolchic.component.core.coolchic import exp_golomb_nbins
...
from coolchic.nnquant.quantstep import POSSIBLE_Q_STEP, get_q_step_from_parameter_name
...
def quantize_model(
    ...
    for cc_name, cc_enc in frame_encoder.coolchic_enc.items():
        ...
        for module_name, cur_module in sorted(module_to_quantize.items()):
            ...
            all_q_step = POSSIBLE_Q_STEP.__getattribute__(module_name)
            all_expgol_cnt = POSSIBLE_EXP_GOL_COUNT.__getattribute__(module_name)

            # Save full precision parameter.
            fp_param = cur_module.get_param()

            best_q_step = DescriptorNN()
            # Overall best expgol count for this module weights and biases
            final_best_expgol_cnt = DescriptorNN()

            for q_step_w, q_step_b in itertools.product(all_q_step.weight, all_q_step.bias):
                ...
                cc_enc.nn_q_step.__setattr__(module_name, current_q_step)
                ...
                for weight_or_bias in ["weight", "bias"]:
                    ...
                    for parameter_name, parameter_value in param.items():
                        ...
                        current_sent_param = (
                            parameter_value / current_q_step.__getattribute__(weight_or_bias)
                        ).view(-1)
                    ...
                    for expgol_cnt in all_expgol_cnt.__getattribute__(weight_or_bias):
                    ...
                    best_expgol_cnt.__setattr__(weight_or_bias, int(cur_best_expgol_cnt))

                cc_enc.nn_expgol_cnt.__setattr__(module_name, best_expgol_cnt)
            ...
            cc_enc.nn_q_step.__setattr__(module_name, best_q_step)
            cc_enc.nn_expgol_cnt.__setattr__(module_name, final_best_expgol_cnt)

            q_param = _quantize_parameters(fp_param, cc_enc.nn_q_step.__getattribute__(module_name))
            assert q_param is not None, (
                "_quantize_parameters() failed with q_step "
                f"{cc_enc.nn_q_step.__getattribute__(module_name)}"
            )
    ...
    frame_encoder.encoder_monitor.increment_time(time_nn_quantization)
