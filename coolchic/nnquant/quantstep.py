...
POSSIBLE_Q_STEP_SHIFT_ARM = DescriptorNN(
    weight=torch.linspace(-8, 0, 9, device="cpu", dtype=torch.int32),
    bias=torch.linspace(-16, 0, 17, device="cpu", dtype=torch.int32),
)


POSSIBLE_Q_STEP = DescriptorCoolChic(
    arm=DescriptorNN(
        weight=2.0**POSSIBLE_Q_STEP_SHIFT_ARM.weight,
        bias=2.0**POSSIBLE_Q_STEP_SHIFT_ARM.bias,
    ),
    ifce=DescriptorNN(
        weight=2.0**POSSIBLE_Q_STEP_SHIFT_ARM.weight,
        bias=2.0**POSSIBLE_Q_STEP_SHIFT_ARM.bias,
