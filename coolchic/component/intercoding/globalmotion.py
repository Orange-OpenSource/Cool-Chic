...
from coolchic.component.intercoding.warp import vanilla_warp_fn
from coolchic.io.format.yuv import convert_420_to_444, convert_444_to_420
from coolchic.io.framedata import FrameData
from coolchic.training.loss import _compute_mse
...
def apply_global_translation(
    reference_frames: List[Tensor],
    global_flows: List[Tensor],
) -> List[Tensor]:
    shifted_ref = []

    for ref_i, global_flow_i in zip(reference_frames, global_flows):
        ups_global_flow_i = F.interpolate(global_flow_i, size=ref_i.size()[-2:], mode="nearest")
        shifted_ref.append(vanilla_warp_fn(ref_i, ups_global_flow_i, mode="nearest"))
    return shifted_ref
