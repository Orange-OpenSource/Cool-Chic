...
class WarpParameter:
    ...
    def __post_init__(self):
        ...
        match self.filter_size:
            case 2:
                self.mode = "torch_bilinear" if self.use_torch_if_available else "bilinear"
            case 4:
                self.mode = "torch_bicubic" if self.use_torch_if_available else "bicubic"
            case _:
                self.mode = "sinc"
...
class Warper(nn.Module):
        ...
        # some already offered by PyTorch e.g. bilinear and bicubic
            ...
            if self.param.mode == "sinc":
                ...
                self.register_buffer(
                    ...
                    torch.arange(left_top_neighbor, right_bot_neighbor + 1).view(1, -1, 1, 1),
                    persistent=False,
                )
    ...
    def get_coeffs(self, s: Tensor) -> Tensor:
        ...
        if self.param.mode in ["sinc"]:
            ...
            # Compensation in Learned Video Codecs".
            window = torch.cos(torch.pi * (s - self.relative_neighbor_idx) / self.filter_size)
            ...
            s_exponents = torch.cat([torch.pow(s, exponent=expo) for expo in range(max_deg)], dim=1)
    ...
    def forward(self, x: Tensor, flow: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            q_flow = (
                torch.round(flow * self.param.fractional_accuracy) / self.param.fractional_accuracy
            )
        ...
        if self.native_pytorch_warp:
            ...
            normalized_neighbors = torch.cat(
                [
                    # fmt: off
                    2 * torch.clamp(neighbors[:, 0:1, :, :], min=0, max=W - 1) / (W - 1) - 1,
                    2 * torch.clamp(neighbors[:, 1:2, :, :], min=0, max=H - 1) / (H - 1) - 1,
                    # fmt:on
                ],
                dim=1,
            )
            ...
            interpolated_lines = self.interpolate_1d(stacked_lines, fractional_flow[:, 0:1])
...
if __name__ == "__main__":
    ...
    for filter_size in [2, 4]:
        ...
        warp_torch = vanilla_warp_fn(dummy_img, dummy_flow, mode=mode.replace("torch_", ""))
