...
class UpsamplingSeparableSymmetricConv2d(nn.Module):
    ...
    def __init__(self, kernel_size: int):
        ...
        assert kernel_size % 2 == 1, f"Upsampling kernel size must be odd, found {kernel_size}."
...
class Upsampling(nn.Module):
    ...
    * If ``ups_k_size >= 8``, a bicubic kernel (with zero padding if
    ...
    def forward(self, decoder_side_latent: List[Tensor]) -> Tensor:
        ...
        The output features ordering are the same than the ordering of the
        input decoder_side_latent.
...
def fixed_upsampling(
    decoder_side_latent: List[Tensor], mode: str = "bicubic"
    ...
    intermediate_reconstructions = [torch.zeros_like(upsampled_latent)]
    for target_tensor in latent_reversed[1:]:
        intermediate_reconstructions.append(upsampled_latent)

        # Our goal is to upsample <upsampled_latent> to the same resolution than <target_tensor>
        # but only if necessary
        if upsampled_latent.shape[2:] != target_tensor.shape[2:]:
            x = rearrange(upsampled_latent, "b c h w -> (b c) 1 h w")
    ...
    return upsampled_latent, intermediate_reconstructions
