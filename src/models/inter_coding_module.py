...
from encoding_management.coding_structure import FRAME_TYPE
from models.coolchic_encoder import CoolChicEncoderOutput
...
class InterCodingModule(nn.Module):
    ...
    def process_coolchic_output(self, coolchic_output: CoolChicEncoderOutput) -> InterCodingModuleInput:
        """Convert the raw, dense tensor from CoolChicEncoder forward into the different
        quantities expected by the InterCodingModule.

        Args:
            coolchic_output (CoolChicEncoderOutput): Dense [B, X, H, W] tensor from CoolChic.
        ...
        if self.frame_type == 'B':
            ...
            beta = torch.clamp(raw_coolchic_output[:, 8:9, :, :] + 0.5, 0., 1.)
        else:
            flow_2 = torch.zeros((b, 2, h, w), device=residue.device)
            beta = torch.ones((b, 1, h, w), device=residue.device)

        return InterCodingModuleInput(residue=residue, flow_1=flow_1, flow_2=flow_2, alpha=alpha, beta=beta)
    ...
    def forward(
        ...
        # Replace "missing" references by zeroed frames
        while len(references) < 2:
            references.append(torch.zeros_like(input_inter_coding.residue))

        # Prediction obtained by weighting two intermediate predictions
        prediction = input_inter_coding.beta * warp(references[0], input_inter_coding.flow_1) \
            + (1 - input_inter_coding.beta) * warp(references[1], input_inter_coding.flow_2)
        ...
        return InterCodingModuleOutput(decoded_image=decoded_frame, additional_data=additional_data)
