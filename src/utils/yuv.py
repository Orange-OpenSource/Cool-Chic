...
from encoding_management.coding_structure import FRAME_DATA_TYPE, POSSIBLE_BITDEPTH, DictTensorYUV, FrameData
...
def convert_444_to_420(yuv444: Tensor) -> DictTensorYUV:
    ...
    The U and V tensors are down sampled using a nearest neighbor downsampling

    Args:
        yuv444 (Tensor): YUV444 4D tensor
...
def convert_420_to_444(yuv420: DictTensorYUV) -> Tensor:
    """Convert a DictTensorYUV (see format above) to a 4D tensor [B, 3, H, W].
    ...
    Returns:
        Tensor: YUV444 Tensor
    """
    u = F.interpolate(yuv420.get('u'), scale_factor=(2, 2))
    v = F.interpolate(yuv420.get('v'), scale_factor=(2, 2))
    video = torch.cat((yuv420.get('y'), u, v), dim=1)
    return video



def write_yuv(data: FrameData, filename: str, norm: bool = True):
    ...
    """
    assert data.frame_data_type in ['yuv420', 'yuv444'], 'Found incorrect datatype in ' \
